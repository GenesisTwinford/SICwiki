  ...
  return (
    <main className="min-h-screen bg-[linear-gradient(180deg,#f0fdf4_0%,#ffffff_50%,#eff6ff_100%)] px-4 py-10">
      <div className="mx-auto grid min-h-[calc(100vh-5rem)] max-w-5xl items-center gap-8 lg:grid-cols-[1.05fr_0.95fr]">
        ...
        <section className="rounded-[32px] border border-black/10 bg-white/90 p-6 shadow-[0_25px_70px_-35px_rgba(15,23,42,0.35)] sm:p-8">
          <div className="space-y-4">
            <div>
              <h2 className="text-2xl font-semibold text-slate-950">新規登録</h2>
              <p className="mt-2 text-sm text-slate-600">
                表示名、メールアドレス、パスワードを設定するとすぐに始められます。
              </p>
            </div>
            ...
            <button
              ...
              disabled={isLoading}
              className="flex w-full items-center justify-center gap-3 rounded-2xl border border-slate-300 px-4 py-3 text-sm font-medium text-slate-700 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50"
            >
            ...
            <form onSubmit={onSubmit} className="space-y-4">
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="w-full rounded-2xl bg-emerald-700 px-4 py-3 text-sm font-medium text-white transition hover:bg-emerald-600 disabled:cursor-not-allowed disabled:opacity-50"
              >
                {isLoading ? "登録中..." : "新規登録"}
              </button>
            </form>
            ...
            <p className="text-sm text-slate-600">
              すでにアカウントを持っている場合は{" "}
              <Link href="/signin" className="font-medium text-emerald-700 underline">
                ログイン
              </Link>
            </p>
          </div>
